...
module.exports = class Catalog {
  ...
  getFileUidFromRequest(request) {
    const arrayToHash = [request.url, request.headers, request.rawHeaders, request.method];
  ...
  find(host, request) {
    ...
    if (!File.existsSync(pathname)) pathname = `${this.getFixturesDir()}/${host}/${requestUID}`;
